...
export async function GET(request: NextRequest) {
  try {
    ...
    if (whereConditions.length > 0) {
      ...
      const totalResult = await db.select({ count: schema.users.id })
        .from(schema.users)
        .where(whereConditions.length === 1 ? whereConditions[0] : undefined);
      ...
      const totalResult = await db.select({ count: schema.users.id }).from(schema.users);
      total = totalResult.length;
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Sanitize data
    const sanitizedData = sanitiseData(userData);
  ...
  try {
    ...
    const updatedUser = await db.update(schema.users)
      .set({
        ...
        isActive: sanitizedData.isActive !== undefined ? Boolean(sanitizedData.isActive) : undefined,
      })
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (permanent) {
      ...
      const deactivatedUser = await db.update(schema.users)
        .set({ isActive: false })
        .where(eq(schema.users.id, parseInt(id)))
        .returning();
