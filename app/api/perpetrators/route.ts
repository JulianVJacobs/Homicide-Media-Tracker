...
export async function GET(request: NextRequest) {
  try {
    ...
    if (articleId) {
      ...
      perpetrators = await db.select()
        .from(schema.perpetrators)
        .where(eq(schema.perpetrators.articleId, articleId))
        .limit(limit)
        .offset(offset);
      
      const totalResult = await db.select({ count: schema.perpetrators.id })
        .from(schema.perpetrators)
        .where(eq(schema.perpetrators.articleId, articleId));
      ...
      perpetrators = await db.select()
        ...
        .where(
          or(
            ...
            like(schema.perpetrators.perpetratorRelationshipToVictim, `%${search}%`),
      ...
      // Get total count
      const totalResult = await db.select({ count: schema.perpetrators.id }).from(schema.perpetrators);
      total = totalResult.length;
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const newPerpetrator = await db.insert(schema.perpetrators).values({
      articleId: sanitizedData.articleId,
      perpetratorName: sanitizedData.perpetratorName,
      perpetratorRelationshipToVictim: sanitizedData.perpetratorRelationshipToVictim,
    ...
    if (isConstraintError) {
      return NextResponse.json(
        { success: false, error: 'Invalid article ID - article does not exist' },
        { status: 409 }
      );
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const updatedPerpetrator = await db.update(schema.perpetrators)
      .set({
        ...sanitizedData,
        updatedAt: new Date().toISOString(),
        syncStatus: 'pending',
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const deletedPerpetrator = await db.delete(schema.perpetrators)
      .where(eq(schema.perpetrators.id, parseInt(id)))
      .returning();
