...
export async function GET(request: NextRequest) {
  try {
    ...
    if (articleId) {
      ...
      victims = await db.select()
        .from(schema.victims)
        .where(eq(schema.victims.articleId, articleId))
        .limit(limit)
        .offset(offset);
      
      const totalResult = await db.select({ count: schema.victims.id })
        .from(schema.victims)
        .where(eq(schema.victims.articleId, articleId));
      ...
      // Get total count
      const totalResult = await db.select({ count: schema.victims.id }).from(schema.victims);
      total = totalResult.length;
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const newVictim = await db.insert(schema.victims).values({
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const updatedVictim = await db.update(schema.victims)
      .set({
        ...sanitizedData,
        updatedAt: new Date().toISOString(),
        syncStatus: 'pending',
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const deletedVictim = await db.delete(schema.victims)
      .where(eq(schema.victims.id, parseInt(id)))
      .returning();
