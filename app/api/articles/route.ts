...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) {
      ...
      const totalResult = await db.select({ count: schema.articles.id })
        .from(schema.articles)
        .where(
          or(
            like(schema.articles.newsReportHeadline, `%${search}%`),
      ...
      // Get total count
      const totalResult = await db.select({ count: schema.articles.id }).from(schema.articles);
      total = totalResult.length;
    }
  ...
  try {
    ...
    if (duplicates.length > 0 && duplicates[0].confidence === 'high') {
      return NextResponse.json({
        ...
        articleId: duplicates[0].articleId,
      }, { status: 409 });
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const [updatedArticle] = await db.update(schema.articles)
      .set(updateData)
      .where(eq(schema.articles.articleId, body.articleId))
      .returning();
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const existingArticle = await db.select()
      .from(schema.articles)
      .where(eq(schema.articles.articleId, articleId))
      .limit(1);
    ...
    await db.delete(schema.victims)
      .where(eq(schema.victims.articleId, articleId));
      
    await db.delete(schema.perpetrators)
