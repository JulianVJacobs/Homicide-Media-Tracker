...
export async function GET(request: NextRequest) {
  try {
    ...
    // Basic counts
    const articleCount = await db.select({ count: sql<number>`count(*)` }).from(schema.articles);
    const victimCount = await db.select({ count: sql<number>`count(*)` }).from(schema.victims);
    const perpetratorCount = await db.select({ count: sql<number>`count(*)` }).from(schema.perpetrators);
    ...
    const articlesByDate = await db
      ...
      .where(sql`datetime(${schema.articles.dateOfPublication}) >= datetime(${daysAgo.toISOString()})`)
    ...
    const dataQuality = {
      articlesWithoutAuthor: await db
        ...
        .from(schema.articles)
        .where(sql`${schema.articles.author} IS NULL OR ${schema.articles.author} = ''`),
      victimsWithoutLocation: await db
        .select({ count: sql<number>`count(*)` })
        .from(schema.victims)
        .where(sql`${schema.victims.placeOfDeathProvince} IS NULL AND ${schema.victims.placeOfDeathTown} IS NULL`),
      victimsWithoutAge: await db
        .select({ count: sql<number>`count(*)` })
        .from(schema.victims)
        .where(sql`${schema.victims.ageOfVictim} IS NULL AND ${schema.victims.ageRangeOfVictim} IS NULL`),
      unidentifiedSuspects: await db
        .select({ count: sql<number>`count(*)` })
        .from(schema.perpetrators)
        .where(sql`${schema.perpetrators.suspectIdentified} = 'No' OR ${schema.perpetrators.suspectIdentified} IS NULL`)
    };
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        sync: syncStats,
        dataQuality: {
          articlesWithoutAuthor: dataQuality.articlesWithoutAuthor[0]?.count || 0,
          victimsWithoutLocation: dataQuality.victimsWithoutLocation[0]?.count || 0,
